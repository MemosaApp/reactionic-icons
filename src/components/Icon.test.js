...
describe('<Icon />', () => {
  ...
  it('does not pass down invalid props', () => {
    const options = {
      context,
    };
    ...
    expect(wrapper.find('i').props()).not.have.key('hidden');
    expect(wrapper.find('i').props()).not.have.key('label');
    expect(wrapper.find('i').props()).not.have.key('icons');
    expect(wrapper.find('i').props()).not.have.key('defaultIcon');
    expect(wrapper.find('i').props()).have.all.keys('aria-label', 'className', 'onClick');
  });
});
